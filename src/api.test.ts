...
describe('api tests', () => {
  ...
  it("api returns empty set", done => {
    ...
    mock.onGet(`${import.meta.env.VITE_API_BASE_URL}/links`).reply(200, data);
  ...
  it("api returns new shortened url", done => {
    ...
    mock.onPost(`${import.meta.env.VITE_API_BASE_URL}/links`).reply(201, data);
  ...
  it("api deletes new shortened url", done => {
    let mock = new MockAdapter(axios);
    mock.onDelete(`${import.meta.env.VITE_API_BASE_URL}/links`).reply(204);
